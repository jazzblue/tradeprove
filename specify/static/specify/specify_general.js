...
function add_graphic_attributes (object_class, object, graphic_table, func_argument) {
    // Sets CSS attributes of the object.
    // Arguments:
    //      object_class - object class
    //      object -  object
    //      graphic_table -  table from which CSS attributes are retrieved
    //      func_argument -  function arguments, in case the attribute is a function

	for(var section in graphic_table[object_class]) {
		...
		if (section_words[0]  ===  "subobject") { 	// Check if section name starts with the word "object".
			...
			add_graphic_attributes(section, subobject, graphic_table[object_class], func_argument);
		}
		else {  // Add attributes to the object
		
			var attribute_value;
			for(var attribute_name in graphic_table[object_class][section]) {
			
				attribute_value = graphic_table[object_class][section][attribute_name];
			
				if (section  ===  "init_dimensions") { 
