...
function offset_first_cell(measure, element_class) {
    // Returns offset measure or original measure if no offset applied.
    // Offset is applied based on first element type.
    // Arguments:
    //      measure: measure to offset.
...
function create_area(area_class) {
	...
	var element_area = $("<div/>")
		...
		add_geometry(area_class, element_area, element_geometry_table, null);
...
function create_cell(element_class, parent_class, element_id) {
    ...
    //      Arguments:
    //          element_class: class of an element belonging to cell.
    //          parent_class: class of element's parent (formula class).
...
function create_fields(element_class, parent_class) {
    ...
    // Arguments:
    //      element_class: class of element that fields belong to.
    //      parent_class: class of parent of element that fields belong to.
	...
	if (element_cfg_table[element_class]["has_fields"]) {
		...
		add_geometry(element_class, element_fields, 
                                          element_geometry_table, parent_class);
...
function create_ctrl_panel(element_class, element_fields, parent_class) {
	// Creates and returns control panel for element
    // Control panel is attached to element fields and allows element 
    // addition before or after the element as well as deletion of an element.
    // Arguments:
    //      element_class: element class
...
function select_exclusive(fields) {
	// Creates visualization for exclusive formula selection.
    // Arguments:
    //      fields: fields associated with exclusive formula
...
function get_cell_id(element_class, parent_class, parent_id, element_idx) {
    ...
    //      element_idx: element index in formula
...
function enum_and_place_formula(formula, offset) {
	...
	formula.children().each(function(idx) {
		...
		if (!element_cfg_table[formula.parent().attr("class")]["formula_exclusive"]) {
			...
			place_cell($(this), formula.parent().attr("class"), idx, cur_cell_offset);
...
function place_cell(cell, formula_element_class, idx, cell_offset) {
    // Places cell: applies offset, width and height  to cell.
    // Arguments:
	...
	if (!element_cfg_table[formula_element_class]["formula_exclusive"]) {
		...
		if (element_cfg_table[formula_element_class]["is_formula_horizontal"]) {
            ...
            cell_width = get_cell_length(formula_element_class, idx);
            cell_height = element_cfg_table[formula_element_class]["common_cell_length"];
		}
		else {
            ...
            cell_width = element_cfg_table[formula_element_class]["common_cell_length"];
            cell_height = get_cell_length(formula_element_class, idx);            
		}
	}
		
	cell.css("top", to_px(cell_top))
		.css("left", to_px(cell_left))
        .css("width", cell_width)
        .css("height", cell_width);
}


function next_cell_offset(cell, formula_element_class, idx, cur_offset) {
    // Returns next cell offset based on cell index and previous cell offset.
    // Arguments:
    //      cell: cell
    //      formula_element_class: formula that cell belongs to.
    //      idx: cell index in the formula.
    //      cur_offset: cell current offset.
	...
	if (!element_cfg_table[formula_element_class]["formula_exclusive"]) {
		...
		next_offset = cur_offset + get_cell_length(formula_element_class, idx) 
                             + (cell.children(".group-open").length 
                             + cell.children(".group-close").length) 
                             * group_margin;
	}
...
function get_cell_length(formula_element_class, idx) {
    // Returns length of the cell.
    // Arguments:
    //      formula_element_class: formula that cell belongs to.
    //      idx: cell index in the formula.

    if (is_operand(idx, formula_element_class)) {   // If the cell contains operand
        cell_length = element_cfg_table[formula_element_class]["operand_cell_length"];
    }
	else {   // If the cell contains operator
